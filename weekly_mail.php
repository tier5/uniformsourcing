...
.'emp.firstname,emp.lastname,prch.purchaseorder,prc.prjquote,prc.prjcost,prc.prj_completioncost,prc.prj_est_profit,prc.prj_estimatecost,'
  ...
  .' where  prch.purchaseorder <> \'\'   and prj.close_date > \''.strtotime("-7 days").'\'';
...
if(isset($datalist[$i]['carrier_id']) && $datalist[$i]['carrier_id']==49)
 {
     $email_body .='<td style="border:1px #FFF solid;">Hand Delivered</td>';
 }
 else
 $email_body .='<td style="border:1px #FFF solid;">'.$datalist[$i]['tracking_number'].'</td>';
        }
 else{       
 if(isset($datalist[$i]['carrier_id']) && $datalist[$i]['carrier_id']==49)
 {
