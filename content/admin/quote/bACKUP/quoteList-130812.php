...
if (isset($_REQUEST['cid']) && $_REQUEST['cid'] != "") {
     $search_sql .=' and q.client_id = ' . $_REQUEST['cid'] . ' ';
}
if (isset($_REQUEST['companyid']) && $_REQUEST['companyid'] != "") {
     $search_sql .=' and q.company_id = ' . $_REQUEST['companyid'] . ' ';
}
if (isset($_REQUEST['q_id']) && $_REQUEST['q_id'] != "") {
     $search_sql .=" and q.po_number ='" . $_REQUEST['q_id'] . "'";
}

$query="SELECT q.*,cmp.company,c.client FROM tbl_request q left join \"clientDB\" c on q.client_id=c.\"ID\" left join tbl_quot_company as cmp on q.company_id = cmp.company_id where q.status = 1 ".$search_sql."  ORDER BY q.qid DESC ";
//echo $query;
                    ...
                    <td width="300px" class="grid001"><select name="cid" id="cid" class="cid" style="width:200px;">
                            ...
                            for ($i = 0; $i < count($data_client); $i++) {
                                $html= "<option value=\"" . $data_client[$i]['ID']."\" "; 
                                       if (isset($_REQUEST['cid']) && $_REQUEST['cid'] ==$data_client[$i]['ID'] )
                           ...
                           <?php 
						  	for($i=0; $i<count($data_cmp); $i++)
							{
                                     $html= "<option value=\"" . $data_cmp[$i]['company_id']."\" "; 
                                       if (isset($_REQUEST['companyid']) && $_REQUEST['companyid'] ==$data_cmp[$i]['company_id'] )
                                   $html.=" selected=\"'selected'\" ";         
                    ...
                    <td class="grid001">
                        <select name="q_id" id="qid" class="pid" style="width:200px;">
                           ...
                           $html= "<option value=\"" . $data_quote_number[$i]['po_number']."\" "; 
                                       if (isset($_REQUEST['q_id']) && $_REQUEST['q_id'] ==$data_quote_number[$i]['po_number'] )
                                   $html.=" selected=\"'selected'\" ";         
