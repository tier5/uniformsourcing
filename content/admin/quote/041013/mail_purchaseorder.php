...
if(isset($_POST['email']) && isset($_POST['subject']))	
{
	...
	$to_count = count($to);
	$to[count($to)] = "accounting@uniforms.net";
	for($k=0 ; $k < count($to); $k++)
	{
		...
		if($isMailServer == 'true')
		{
			...
			$mail->AddReplyTo("admin@uniformsourcing.com", $name);
		...
		else
		{
			...
			$headers=create_smtp_headers($subject, "admin@uniformsourcing.com", $to[$k], 'Uniform Sourcing',"","text/html");
			$data=$headers. "<body>".$body."</body>";
