...
if(isset($_POST['submit']))
{
	...
	if($qid == 0)
	{
		$query="select nextval('tbl_request_qid_seq'::regclass) as qid ";
	...
	if($_POST['submit'] == 'Add')
	{
		$log_desc ="New Quote Added";
  		$query="INSERT INTO tbl_request(qid, status";
	...
	for($i = 0; $i < count($item);$i++)
	{
		...
		{
			$query .= "Update tbl_request_items SET status =1";
			$query .= ", qid = $qid ";
			$query .= ", itemno = '$item[$i]' ";
		...
		{
			$query .="INSERT INTO tbl_request_items ( ";
			$query .=" qid ";
			$query .=", itemno ";
			$query .=", description ";
			...
			if($tax_amount[$i]!=""){$query .=", tax_amount ";}
			if($amount[$i]!=""){$query .=", amount ";}
			$query .=")";
			$query .=" VALUES (";
			$query .=" '$qid' ";
			$query .=", '{$item[$i]}' ";
			...
			if($tax_amount[$i]!="")$query .=", {$tax_amount[$i]} ";
			if($amount[$i]!="")$query .=", {$amount[$i]} ";
			$query .="); ";
		}
	}
	if($query != "")
