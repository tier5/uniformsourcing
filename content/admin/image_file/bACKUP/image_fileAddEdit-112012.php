...
//print_r($data_prjUploads);
	...
	while($row = pg_fetch_array($result)){
		$data_client[] =$row;
         ...
         $sql='select style.style_id,invstyle."styleNumber" from img_file_styles as style left join "tbl_invStyle" as invstyle on invstyle."styleId"=style.style_id where pack_id='.$_GET['pack_id'];
       //echo $sql;
	if(!($result=pg_query($connection,$sql))){
		print("Failed query1: " . pg_last_error($connection));
		exit;
...
<td align="left">
<select id="style"  onchange="javascript:addStyle()" ><option value="">--Select--<option>
   <?php  foreach ($data_styleid as $style) {
...
<td valign="top">
    ...
    {
  echo '<tr height="30" id="st_'.$data_style[$i]['style_id'].'"><td>'.$data_style[$i]['styleNumber'].'<input type="hidden" name="style[]" value="'.$data_style[$i]['style_id'].'"/></td>';      
  echo '<td><img width="20" height="20" src="'.$mydirectory.'/images/close.png" alt="delete"'.
    ' onclick="javascript:$(\'#st_'.$data_style[$i]['style_id'].'\').remove();"/></td></tr>';
    }
    ?>
    </table> 
</td>
...
function addStyle()
   ...
   cl_html='<tr height="30" id="st_'+$("#style").val()+'"><td>'+$("#style :selected").text()+'<input type="hidden" name="style[]" value="'+$("#style").val()+'"/></td>';
cl_html+='<td><img width="20" height="20" src="<?php echo $mydirectory;?>/images/close.png" alt="delete"';
