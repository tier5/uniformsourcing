...
$wodate=mktime(0, 0, 0, date("m"), date("d"), date("Y"));
$code=$_POST['code'];
if($debug == "on"){
	echo "code IS $code<br>";
}
$query1=("SELECT \"ID\", \"clientID\", \"employeeID\", \"itemid\", \"hours\", \"total\", \"whoassigned\", \"jobnotes\" ".
		 "FROM \"work1\" ".
		 "WHERE \"ID\" = '$code'");
...
$query3=("INSERT INTO \"timelog\" ".
		 "(\"workid\", \"employee\", \"wodate\", \"start\", \"end\", \"total\", \"description\", \"billed\", \"client\", \"cost\", \"rate\") ".
		 "VALUES ('".$data1[0]['itemid']."', '".$data1[0]['employeeID']."', '$wodate', '$start', '$end', '".$data1[0]['total']."', '".$data1[0]['jobnotes']."', 'no', '".$data1[0]['clientID']."', '$cost', '".$data2[0]['price']."')");
