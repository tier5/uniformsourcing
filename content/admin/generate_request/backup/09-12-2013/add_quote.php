...
if($isEdit)
{
	...
	$query="Select * from tbl_quote_items where status = 1 and qid = $qid";
	if(!($result=pg_query($connection,$query))){
		print("Failed tax_query: " . pg_last_error($connection));
		exit;
	}
...
?>
                            ...
                            <tr>
                              ...
                              <td>&nbsp;</td>
                              <td align="left"><input name="project_name" type="text" value="<?php echo $data_quote['project_name'];?>" /></td>
                            </tr>
                            <tr>
...
?>
                                ...
                                <table width="100%" border="0" cellpadding="0" cellspacing="0" id="ItemTbl">
                                  ...
                                  <tr>
                                    <td width="50%" height="25" align="right"> Ship Via:</td>
                                    <td width="1%">&nbsp;</td>
...
?>
                                  ...
                                  <tr>
                                    ...
                                    <td>&nbsp;</td>
                                    <td align="left">
                                    <input name="qid" id="qid" type="hidden" value="<?php echo $data_quote['qid'];?>" />
...
function popOpen(count)
{
	...
	if(count != null || count > 0)
	{
		...
		document.getElementById('tax_amount_id').value =Math.round(Number(document.getElementById('tax_id'+count).value)*100)/100;
		...
		document.getElementById('amount_id').value =Math.round(Number(document.getElementById('amount_id'+count).value)*100)/100;
...
function calculateSub(tableid)
{
		...
		document.getElementById('amountsubtotal_id').value = Math.round(subAmountTotal, 2);
		document.getElementById('taxsubtotal_id').value = Math.round(subTaxTotal, 2);;
		document.getElementById('total_id').value = Math.round(total, 2);
}
function calculateAmount()
{
		...
		if(taxtype!="" && taxtype >0)
			...
			tax_type = 0
		var taxamount = Math.round(unit_price * quant * tax_type*100)/100;
		var amount = Math.round(unit_price * quant*100)/100;
