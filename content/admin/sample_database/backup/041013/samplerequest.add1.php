...
if(isset($_POST['id']))
{
	...
	$sql="Select count(*) as n from \"tbl_sample_database\" where \"sample_id_val\"='$srID'";
	if($id >0)
	...
	if($id == 0)
	{
		...
		if($srID !="") $query4.=", sample_id_val ";
		if($style!="") $query4.=", style_number ";
		if($briefdesc !="") $query4.=", brief_desc ";
		if($size !="") $query4.=", size ";
		if($quantity !="") $query4.=", quantity ";
		if($detaildesc !="") $query4.=", detail_description ";
		if($vendorID) $query4.=",vid ";
		if($color!="") $query4.=", color ";
		if($fabricType!="") $query4.=", fabric";
		if($cost!="") $query4.=", samplecost ";
		if($retailprice!="") $query4.=", retailprice ";
		if($inStock!="") $query4.=", instock ";
		if($embroidery!="") $query4.=", embroidery_new ";
		if($silkscreening!="") $query4.=", silkscreening ";
		if($clientname!="") $query4.=", client ";
		if($location!="") $query4.=", location ";
		if($sample_type>0) $query4.=", sample_type_id ";
		$query4.=")";
		...
		if($silkscreening!="") $query4.=" ,'$silkscreening' ";
		if($clientname!="") $query4.=" ,'$clientname' ";
		if($location!="") $query4.=" ,'$location' ";
		if($sample_type>0) $query4.=" ,$sample_type ";
		$query4.=")";
	...
	else if($id > 0)
	{
		...
		else  $query4.="brand_manufct  = null, ";	
		if($srID!= "") $query4.="sample_id_val = '$srID', ";
		else $query4.="sample_id_val = null, ";
		if($style!="") $query4.="style_number = '$style', ";
		else $query4.="style_number = null, ";
		if($briefdesc !="")  $query4.="brief_desc = '$briefdesc', ";
		else  $query4.="brief_desc = null, ";
		if($size!="")   $query4.="size = '$size', "; 
		else $query4.="size = null, ";
		if($quantity!="")   $query4.="quantity = '$quantity', "; 
		else $query4.="quantity = null, "; 
		if($detaildesc !="") $query4.="detail_description = '$detaildesc', "; 
		else $query4.="detail_description = null, "; 
		if($vendorID >0) $query4.="vid = '$vendorID', ";
		else $query4.="vid = 0, ";
		if($color!="") $query4.="color = '$color', ";
		else $query4.="color = null, ";
		if($fabricType!="") $query4.="fabric = '$fabricType', ";
		else $query4.="fabric = null, ";
		if($cost!="") $query4.="samplecost = '$cost', ";
		else $query4.="samplecost = null, ";
		if($retailprice!="") $query4.="retailprice = '$retailprice', ";
		else $query4.="retailprice = null, ";
		if($inStock!="")  $query4.="instock = '$inStock', ";
		if($embroidery!="") $query4.="embroidery_new = '$embroidery', ";
		if($clientname!="") $query4.="client = '$clientname', ";
		else $query4.="client = null, ";
		if($location!="") $query4.="location = '$location', ";
		else $query4.="location = null, ";
		if($silkscreening!="") $query4.=" silkscreening = '$silkscreening', ";
		if($sample_type>0) $query4.=" sample_type_id = '$sample_type', ";
               ...
               ' <tr>'.
				...
				else
				{
					...
					if($is_mail ==1 && $clientEmail !="")
					{
						...
						if((send_smtp($mailServerAddress,"admin@uniformsourcing.com",$samplerequest_emailing, $data)) == false)
						{
							$return_arr['error'] = "Unable to send email. Please try again later";
							echo json_encode($return_arr);
							return;
