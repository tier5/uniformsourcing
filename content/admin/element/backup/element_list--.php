...
{
 if($where=="")
      $where.=" where package.client=".$_GET['client']; 
 else
...
{
 if($where=="")
      $where.=" where package.style like '%".$_GET['style']."%'"; 
 else
...
$query= 'Select distinct main.*,(select client.client from tbl_element_package as pack left join "clientDB" as client '.
 ...
 ' where pack_id= main.pack_id limit 1) as client_name from "tbl_element_pack_main" as main,tbl_element_package as package  '.$where.
