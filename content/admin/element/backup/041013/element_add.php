...
//print_r($datalist);
$sql = 'Select "vendorID","vendorName" from "vendor" order by "vendorName" ';
...
if (isset($_GET['element_id'])&&$_GET['element_id']!="") {
$sql = 'Select "pack_name",client from "tbl_element_pack_main" where pack_id='.$element_id;;
if (!($result = pg_query($connection, $sql))) {
    print("Failed query1: " . pg_last_error($connection));
    exit;
}
...
$element_list=array("Artwork","Beads","Boning","Buttons","Care Label","Chips","Chip Pouch","CMT","Crystals","Company Label","Cups/Pads","Embroidery","Fabric","Grade","Hang Tag","Heat Press","Hook & Eye","Interfacing","Lap Dip","Lacing","Liner","Marker","Other","Packaging","Pattern","Pocketing","Screen Printing","Size Label","Snaps","Studs","Sublimation","Thread","Trim &Piping","Underwire","Zipper");
...
<div class="content" align="center">
    <form action="element_submit.php" method="post" id="package_form">
      ...
      <td align="left"><input style="width:225px;" type="text" name="packagename" id="packagename" value="<?php
      if(isset($package_name))
      echo stripslashes($package_name); ?>"/></td>
        </tr>
        
        <?php echo'<tr><td align="right">Client:</td><td align=\'left\'><select style="width:232px;" id=\'client\' name=\'client\'>';
        echo '<option selected="selected" value="">--Select--</option>';
for($i=0; $i < count($client); $i++){
     if(isset($cl) && $cl==$client[$i]['ID'])
...
echo $pid;?>')" src="../../images/delete.png">
	...
	<td align="left"><select name="elementtype[]" <?php echo $style_price;?> id="elementtype<?php echo $j;?>" onChange="showlocation('<?php echo $j;?>');">
...
<option value="<?php echo $element_list[$i];?>"><?php echo $element_list[$i];?></option>
  ...
  <?php 
  if($datalist[$j]['element_type']=='Boning'||$datalist[$j]['element_type']=='Boning'||$datalist[$j]['element_type']=='Buttons'||$datalist[$j]['element_type']=='CMT'||
$datalist[$j]['element_type']=='Grade'||$datalist[$j]['element_type']=='Lap Dip'||$datalist[$j]['element_type']=='Liner'||$datalist[$j]['element_type']=='Marker'
||$datalist[$j]['element_type']=='Packaging'||$datalist[$j]['element_type']=='Pattern'||$datalist[$j]['element_type']=='Underwire')
  {?>
   <tr id="location_tr<?php echo $j;?>" style='display:none;'>
    <td align="right">Location:</td>
    <td align="left"><input type="text" name="location[]" value=""  id="location<?php echo $j;?>"  /></td>
    </tr>
    <tr id="yield_tr<?php echo $j;?>"  style='display:none;' >
    <td align="right">Yield:</td>
    <td align="left"><input type="text" name="yield[]"  value="" id="yield<?php echo $j;?>"  /></td>
    </tr>    
<?php  }else{?>
    <tr id="location_tr<?php echo $j;?>" <?php if($datalist[$j]['element_type']=="Fabric") echo " style='display:none;'"; ?> >
    <td align="right">Location:</td>
    <td align="left"><input type="text" name="location[]" value="<?php echo $datalist[$j]['location'];?>"  id="location<?php echo $j;?>"  /></td>
    </tr>
    <tr id="yield_tr<?php echo $j;?>" <?php if($datalist[$j]['element_type']!="Fabric") echo " style='display:none;'"; ?> >
    <td align="right">Yield:</td>
    <td align="left"><input type="text" name="yield[]"  value="<?php echo $datalist[$j]['yield'];?>" id="yield<?php echo $j;?>"  /></td>
    </tr>
...
<?php 	if(!(isset($datalist[$j]['image']) && $datalist[$j]['image']!='')){ ?>
		style="display:none;"
	<?php }?> >
	<td id="img_td<?php echo $count;?>"><strong>Image:</strong><br/>
...
onClick=" javascript: $('#img_td<?php echo $count;?>').removeAttr('src'); document.getElementById('tr_id<?php 
...
in_htm +='<tr><td align="right">';
 ...
 in_htm +='</td></tr>';
in_htm+='<tr><td align="right">Element Type:</td><td align="left"><select id="elementtype'+count+'" onChange="showlocation('+count+');" name="elementtype[]"><?php	
...
/*  in_htm+= " echo'<tr><td>Client:</td><td align=\'left\'><select id=\'client\' name=\'client[]\'>';
for($i=0; $i < count($client); $i++){
    echo '<option value=\''.$client[$i]['ID'].'\'>'.$client[$i]['client'].'</option>';
...
echo '</select></td></tr>';
    ";*/
      ...
      '"><td><div id="img_'+ count +'"><strong>Image:</strong><br/><img id="img_file'+ count +
      '" width="101px" height="89px" src="" onClick="PopEx(this, null,  null, 0, 0, 50, \'PopBoxImageLarge\');" alt="img"/>'+
      '<a id="del_img" style="cursor:hand;cursor:pointer;" onClick=" javascript: $(\'#img_'+count+'\').removeAttr(\'src\'); document.getElementById(\'tr_id'+ count +
      '\').style.display=\'none\'; document.getElementById(\'file_name'+ count +'\').value=\'\'; " >'+
      '<img src="<?php echo $mydirectory; ?>/images/close.png" alt="delete" /></a></div></td>	</tr><tr id="tr_id'+ Number(++count) +
      '" " ></tr></table></td></tr>';
//alert(in_htm);
                  ...
                  success: function (data, status)
                  {
                      ...
                      else
                      {
                              ...
                              else
                              {
                                  ...
                                  $("#tr_id"+data.index).show();       
                                  $("#file_name"+data.index).val(data.name);
                                  $("#tr_id"+data.index).html('<td><strong>'+data.file_name+'</strong>'+
                                      
     '<a id="del_img" style="cursor:hand;cursor:pointer;" onClick=" javascript: $(\'#tr_id'+data.index+'\').html(\'\'); document.getElementById(\'tr_id'+ data.index +
      '\').style.display=\'none\'; document.getElementById(\'file_name'+ data.index +'\').value=\'\'; " >'+
...
{	
//alert($('#elementtype'+i).val());
if($('#elementtype'+i).val()=='Fabric')
{
$('#location_tr'+i).hide();
$('#yield_tr'+i).show();
$('#location'+i).val('');
...
else if($('#elementtype'+i).val()=='Boning'||$('#elementtype'+i).val()=='Boning'||$('#elementtype'+i).val()=='Buttons'||$('#elementtype'+i).val()=='CMT'||
$('#elementtype'+i).val()=='Cups/Pads'||$('#elementtype'+i).val()=='Grade'||$('#elementtype'+i).val()=='Lap Dip'||$('#elementtype'+i).val()=='Liner'||$('#elementtype'+i).val()=='Marker'
||$('#elementtype'+i).val()=='Packaging'||$('#elementtype'+i).val()=='Pattern'||$('#elementtype'+i).val()=='Underwire')
{
    $('#location_tr'+i).hide();
$('#yield_tr'+i).hide();
$('#yield'+i).val('');
