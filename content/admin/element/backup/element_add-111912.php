...
<?php 	for($i=0; $i < count($element_list); $i++){
		if($datalist[$j]['elementtype']==$element_list[$i]){?>
...
<option value="<?php echo $element_list[$i];?>"><?php echo $element_list[$i];?></option>
	...
	for($i=0; $i < count($data_vendor); $i++)
        {
		if($datalist[$j]['vid']==$data_vendor[$i]['vendorID']){?>
<option value="<?php echo $data_vendor[$i]['vendorID'];?>" selected="selected">
    <?php echo $data_vendor[$i]['vendorName']; ?></option>
	<?php 	}else {?>
		<option value="<?php echo $data_vendor[$i]['vendorID'];?>"><?php echo $data_vendor[$i]['vendorName'];?>hh</option>
	<?php }} ?>
</select>
        </td>
	</tr>
	...
	<td align="left">
    <input type="text" name="elementcost[]" id="elementcost" value="<?php echo $datalist[$j]['element_cost'];?>" <?php 
    echo $style_price?>/></td>
	</tr>
	<tr>
...
onClick=" javascript: DeleteUploads('<?php echo $datalist[$j]['prj_element_id']?>','<?php 
echo $count;?>','<?php echo $pid;?>','I','element'); document.getElementById('tr_id<?php 
...
<?php echo (substr($datalist[$j]['file'], (strpos($datalist[$j]['file'], "-")+1)));?>
  ...
  <img src="<?php echo $mydirectory;?>/images/Download.png" alt="download" /></a>
        ...
        <a href="javascript:void(0);" onClick="javascript:DeleteUploads('<?php 
   echo $datalist[$j]['prj_element_id'];?>','<?php echo $count;?>','<?php 
   echo $pid;?>','F','element'); document.getElementById('tr_id<?php 
   echo $count;?>').style.display='none'; document.getElementById('file_name<?php echo $count++;?>').value='';">
   <img src="<?php echo $mydirectory;?>/images/close.png" alt="delete"/>
   </a>
        </td>
