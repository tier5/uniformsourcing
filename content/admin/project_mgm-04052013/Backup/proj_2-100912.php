...
if(isset($_POST['pid']) && $_POST['pid']!=0){
	 $sql = 'select "sent_date","confirm_num","vendor_po","tbl_vendorid",pid,vid,vendor."vendorName" from tbl_prjvendor inner join vendor '
...
if($pid)
    {
        ...
        if(isset($data_prjVendor[$i]['vendor_po']) && $data_prjVendor[$i]['vendor_po']!="")
         ...
         if(isset($data_prjVendor[$i]['sent_date']) && $data_prjVendor[$i]['sent_date']!="")
          $vender .='<tr class="vendor_row_'.$i.'"><td>Date: </td><td>'.$data_prjVendor[$i]['sent_date'].'</td></tr>'; 
