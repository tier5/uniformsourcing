...
for($j = 0; $j < count($data_prj_element_all); $j++)
{
	...
	<td align="left">
	<input type="file" name="file'.$count.'" id="file'.$count.'" onchange="showCloseLink();ajaxFileUpload('.$count.', \'I\', 960,720);" />
	...
	$html .= '" onClick="PopEx(this, null,  null, 0, 0, 50, \'PopBoxImageLarge\');" >';
        if($data_prj_element_all[$j]['img_type']=="CURR"){
           $html .= ' <a id="del_img" style="cursor:hand;cursor:pointer;';
							if($emp_type ==1){
	...
	<tr id="tr_id'.($count).'" ';
	if($data_prj_element_all[$j]['elementfile']=='' || $data_prj_element_all[$j]['elementfile']==NULL){
		$html .= ' style="display:none;" ';
	}
	$html .= '>
	...
	//if(isset($data_prj_element_all[$j]['elementfile']))
        ...
        if($data_prj_element_all[$j]['file_type']=="CURR"){
	...
	$html .= 'href="javascript:void(0);" onClick="javascript:DeleteUploads(\''.$data_prj_element_all[$j]['prj_element_id'].'\',\''.$count.'\',\''.$pid.'\',\'F\',\'element\'); document.getElementById(\'tr_id'.$count.'\').style.display=\'none\'; document.getElementById(\'file_name'.$count++.'\').value=\'\';"><img src="'.$mydirectory.'/images/close.png" alt="delete"/></a>';
        }
        else
        {
            $count++;
