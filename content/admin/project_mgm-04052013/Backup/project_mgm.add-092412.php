...
<li onClick="javascript:load_tab('12'); document.getElementById('message').innerHTML = '';"><a href="#" rel="proj_content12">Inventory</a></li>
<?php
...
function AddElement(){
	...
	cell1.align="center";
	cell1.innerHTML = '<input type="image" class="deleteTd" src="../../images/delete.png" onclick=" DeleteUploads(\'\',\''+ ++count +'\',\'\',\'I\',\'editTime\'); DeleteUploads(\'\',\''+ ++count +'\',\'\',\'F\',\'editTime\');"> <table width="80%" ><tr><td valign="top"><table cellpadding="1" cellspacing="1" border="0"><tr><td align="right">Element Type:</td><td align="left"><select name="elementtype[]"><?php	for($i=0; $i < count($data_elements); $i++){?><option value="<?php echo $data_elements[$i]['element_id'];?>"><?php echo $data_elements[$i]['elements'];?></option><?php } ?></select></td></tr><tr><td align="right">Vendor:</td><td align="left"><select name="vendor_ID[]">	<?php for($i=0; $i < count($data_Vendr); $i++){?><option value="<?php echo $data_Vendr[$i]['vendorID']; ?>"><?php echo $data_Vendr[$i]['vendorName'];?></option><?php } ?></select></td></tr><tr><td align="right">Style:</td><td align="left"><input type="text" name="elementstyle[]" id="elementstyle" value="" /></td></tr><tr><td align="right">Color:</td><td align="left"><input type="text" name="elementcolor[]" id="elementcolor" value="" /></td></tr><tr><td align="right">Cost:</td><td align="left"><input type="text" name="elementcost[]" id="elementcost" value="" /></td></tr><tr><td align="right">Image:</td><td align="left"><input type="file" name="file'+ --count +'" id="file'+ count +'" onchange="javascript:ajaxFileUpload('+ count +', \'I\', 960,720);" /><input type="hidden" id="file_name'+ count +'" name="element_file0[]" value=""/><input type="hidden" id="upload_type'+ count +'" name="element_type0[]" value="I"/><input type="hidden" id="upload_id'+ ++count +'" name="element_id0[]" value=""/></td></tr><tr><td align="right">File:</td><td align="left"><input type="file" name="file'+ count +'" id="file'+ count +'" onchange="javascript:ajaxFileUpload('+ count +', \'F\', 960,720);" /><input type="hidden" id="file_name'+ count +'" name="element_file1[]" value=""/><input type="hidden" id="upload_type'+ --count +'" name="element_type1[]" value="F"/></td></tr></tr></table><input type="hidden" id="element_id" name="element_id[]" value="0"/></td><td valign="top" align="right"><table  border="0" cellspacing="0" cellpadding="0"><tr id="tr_id'+ count +'" style="display:none;"><td><strong>Image:</strong><br/><img id="img_file'+ count +'" width="101px" height="89px" src="" onClick="PopEx(this, null,  null, 0, 0, 50, \'PopBoxImageLarge\');" ><a style="cursor:hand;cursor:pointer;" onClick=" javascript: DeleteUploads(\'\',\''+ count +'\',\'\',\'I\',\'editTime\'); document.getElementById(\'tr_id'+ count +'\').style.display=\'none\'; document.getElementById(\'file_name'+ ++count +'\').value=\'\'; " ><img src="<?php echo $mydirectory; ?>/images/close.png" alt="delete" /></a></td>	</tr><tr id="tr_id'+ count +'" style="display:none;" ></tr></table></td></tr></table>';
...
/*function editStyle(sid, i){
    $.ajax({
           type: 'POST',
           url: 'edit_style.php',
           data:{"sid":sid, "index":i},
           dataType:'json',
           success: function(data){
             // $("#color0").html(data);          
           }
...
/*function multipleStyle_style_tab(id,type)
{
	...
	element8.type = "text";	
	element8.name = "quantity[]";	
	element8.id = "quantity"+rowCount;
