...
//print_r($data_prjUploads);
	...
	while($row = pg_fetch_array($result)){
        ...
        pg_free_result($result);
         $sql='select style.style_id,invstyle."styleNumber" from img_file_styles as style left join "tbl_invStyle" as invstyle on invstyle."styleId"=style.style_id where pack_id='.$data_package_list[$j]['pack_id'];
       //echo $sql;
	if(!($result=pg_query($connection,$sql))){
		print("Failed query1: " . pg_last_error($connection));
		exit;
...
$html.='</td></tr>';
  ...
  $html.='<tr> <td>&nbsp;</td><td colspan="3"  id="pack_view"></td></tr>'.
          '<tr> <td>&nbsp;</td><td colspan="3"><table  border="0" cellspacing="0" cellpadding="0" id="image_view" width="100%"></td></tr></table>';
   $html.='</table>';
...
$html .= '<div style="float:left;" id="tr_id'.$count.'">';
		if($imageArr[$i] != "" )
		{         
$html .= '<img  src="';
