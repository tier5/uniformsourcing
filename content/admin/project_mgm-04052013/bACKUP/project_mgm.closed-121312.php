...
if(isset($_GET['reopen']))
{
 ...
 $tbl_list=array(array('tbl_newproject','rr'),array('tbl_prjpurchase','purchaseId'),array('tbl_prjimage_file','tbl_prjimage_file'),
  array('tbl_prjvendor','tbl_vendorid'),array('tbl_prj_style','prj_style_id'),array('tbl_prj_sample','sample_id'),array('tbl_prjsample_uploads','upload_id'),
...
$sql=' select Distinct(prj.projectname),c.client,prj.pid,prj.order_placeon,prj.status, tbl_prj_style_closed.style,emp.firstname,emp.lastname'.
...
'as prc on prc.pid = prj.pid left join "employeeDB" as emp on emp."employeeID"= prj.project_manager left join "clientDB" c on prj.client=c."ID" '.
  'left join tbl_prj_style_closed on tbl_prj_style_closed.prj_style_id = (select tbl_prj_style.prj_style_id from tbl_prj_style inner '.
 'join tbl_newproject_closed on tbl_prj_style_closed.pid = prj.pid order by tbl_prj_style_closed.prj_style_id desc limit 1) where prch.purchaseorder '
