...
if(isset($_GET['del']) && (strtolower($USERNAME) == 'neal' || strtolower($USERNAME) == 'nicole' || strtolower($USERNAME) == 'rejith'))
{
	...
	$sql .= "delete from tbl_prjsample_notes where pid =".$pid."; ";
	$sql .= "delete from tbl_prjsample_uploads where pid =".$pid."; ";
	$sql .= "delete from tbl_prj_sample where pid =".$pid."; ";
	$sql .= "delete from tbl_prj_sample_po_items where pid =".$pid."; ";
	$sql .= "delete from tbl_prj_sample_po where pid =".$pid."; ";
	$sql .= "delete from tbl_upload_pack where pid =".$pid."; ";
	$sql .= "delete from tbl_prj_style_custom where pid =".$pid."; ";
	$sql .= "delete from tbl_prmilestone where pid =".$pid."; ";
...
$sql=' select Distinct(prj.projectname),prj.is_billed, prj.bill_date,c.client,prj.pid,prj.close_date,prj.order_placeon,prj.status,emp.firstname,emp.lastname '
.',tbl_prj_style.style,prch.purchaseorder,prc.prjquote,prch.purchaseduedate, prc.prjcost,prc.prj_estimatecost,prc.prj_completioncost from '
 .'tbl_newproject as prj inner join tbl_prjpurchase as prch on prch.pid = prj.pid left join tbl_prjpricing as prc on prc.pid = prj.pid '
  .'left join "clientDB" c on prj.client=c."ID" left join "employeeDB" as
  emp on emp."employeeID"= prj.project_manager left join tbl_prj_style on tbl_prj_style.prj_style_id '
 .'= (select tbl_prj_style.prj_style_id from tbl_prj_style inner join tbl_newproject on tbl_prj_style.pid = prj.pid '
  .'order by tbl_prj_style.prj_style_id desc limit 1) where prj.status =0 and prch.purchaseorder <> \'\'  '.$search_sql.' order by prj.close_date desc ';
//echo $sql;
...
if(count($datalist)) 
{
	for($i=0; $i < count($datalist); $i++)
	{
		...
		echo '<td class="grid001B"><a href="#" onclick="javascript:popOpen('.$datalist[$i]['pid'].');"><img src="'.$mydirectory.'/images/bullet_add.png" alt="edit" /></a></td>';
