...
//-------------------------------------------------------------		
  ...
  $tracking_no=$hdn_shipping_id[$i]; //echo $tracking_no."  tt  ";      
     ...
     $q='select max(shipping_id) as sid from tbl_prjorder_shipping '; 
   ...
   $tracking_no=$r['sid'];            
    ...
    $query_Name="delete from tbl_prjorder_track_no where shipping_id=".$tracking_no;
    for($j=0;$track_shipping[$i][$j]!="";$j++)
    {
if(isset($track_shipping[$i][$j])&&$track_shipping[$i][$j]!="")        
   $query_Name.=';insert into tbl_prjorder_track_no(tracking_no,shipping_id) values(\''.$track_shipping[$i][$j].'\','.$tracking_no.')';     
    }
    //echo $query_Name."<br/>";
   if($query_Name!="")
   {
