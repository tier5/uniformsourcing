...
//print_r($data_prjUploads);
        ...
        $query1="SELECT pack_name from  img_file_pack where pack_id=".$pack_id;
...
unset($row);
  pg_free_result($result1);          
	...
	while($row = pg_fetch_array($result)){
        ...
        pg_free_result($result);
 $sql='select style,img_style_id from img_file_styles as style where pack_id='.$pack_id;
       //echo $sql;
	if(!($result=pg_query($connection,$sql))){
		print("Failed query1: " . pg_last_error($connection));
		exit;
 ...
 $upload_dir = "$mydirectory/uploadFiles/image_file/";
  ...
  $html .='<table width="100%" id="upld_pack_'.$pack_id.'"><tr>'.
 ' <td>Style Number: <strong>'.$package_name.'</td> '.'<td valign="top">'. 
 '<img  onclick="$(\'#upld_pack_'.$pack_id.'\').remove()" src="../../images/delete.png" /></td></tr>';
  
  
   $html .='<tr><td width="100%">';
    
   $html .='<input type="hidden" name="upload_packages[]" value="'.$pack_id.'"/>';
    $html .='<table width="100%"><tr><td width="30%">';
