...
$return_arr['error'] = "";
$return_arr['type'] = "";
if (isset($data_inv1[0]['inventoryId'])) {
    $sql = 'select "inventoryId", "styleId", "colorId", "sizeScaleId", price, "locationId","opt1ScaleId", "opt2ScaleId", quantity, "mainSize", "rowSize", "newQty" from "tbl_inventory" where "inventoryId"=' . $data_inv1[0]['inventoryId'] . ' and "isStorage"=0';
    if (!($result = pg_query($connection, $sql))) {
        print("Failed StorageData: " . pg_last_error($connection));
        exit;
    }
    ...
    if ($data_inv['inventoryId'] != "") {
        $sql = 'select "storageId", "invId", "sizeScaleId", "opt1ScaleId", "opt2ScaleId", "locationId", "conveyorSlotId", "conveyorQty", room, "row", rack, shelf, box, "wareHouseQty", "location", "otherQty" from "tbl_invStorage" where "invId"=' . $data_inv1[0]['inventoryId'] . ' order by "storageId"';
