...
if(isset($_GET['type']))
{
	switch($_GET['type'])
	{
		...
		case "e":
		{
			...
			$sql='select * from "tbl_invColor" where "styleId"='.$_GET['ID'].' order by "colorId"  asc';
			if(!($result=pg_query($connection,$sql))){
				print("Failed query1: " . pg_last_error($connection));
				exit;
			}
...
function(data){if(data!=null){if(data.name || data.error){$("#message").html("<div class='errorMessage'><strong>Sorry, " + data.name + data.error +"</strong></div>");}else{if(document.getElementById('isEdit').value==1){$("#message").html("<div class='successMessage'><strong>Style details Updated. Thank you.</strong></div>");}else{$("#message").html("<div class='successMessage'><strong>New Style Information Added. Thank you.</strong></div>");ClearAllFields();$(location).attr('href',"reports.php?ID="+data.id+"&type=a");} } } else{ $("#message").html("<div class='errorMessage'><strong>Sorry, Unable to process.Please try again later.</strong></div>");}} });return false;});});
<?php }?>
