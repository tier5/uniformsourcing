...
$query = '';
$query = 'SELECT unit.id,unit.box,unit.type,details.warehouse,details.container,details.conveyor,location.name,style."styleNumber",style.sex,g."garmentName" FROM "tbl_invUnit" unit'.
    ' LEFT JOIN "tbl_invStyle" style ON style."styleId"=unit."styleId"'.
    ' LEFT JOIN "tbl_garment" g ON style."garmentId" = g."garmentID"'.
    ...
    'WHERE unit."styleId"='.$_GET['styleId'].' and unit.merged=0 ORDER BY unit.id DESC';
                        ...
                        <table width="50%" border="0" cellspacing="1" cellpadding="1">
                            ...
                            if(count($data) > 0) {
                                foreach ($data as $key => $value) {
                                    ?>
                                    <tr>
                                        <td><input type="checkbox" class="checkbox" value="<?php echo $value['id'] ?>" name="checkboxlist"></td>
                                        <td><?php echo $value['name'] ?></td>
                                        <td><?php echo $value[$value['type']] ?></td>
