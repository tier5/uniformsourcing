...
/* The search query*/
	...
	/* Query to search all records within the date limit*/
			...
			if($isAlphaSearch ==1)
			{
				$sql = "SELECT * FROM tbl_prj_style WHERE status=1 and UPPER(style) LIKE '$alphaKey%'";
			}
			if(isset($_REQUEST['fromDate']) && isset($_REQUEST['toDate']))
			{
				if($_REQUEST['fromDate']!="")
				{
					...
					if($toDate=="")
						$toDate=date('U');
				$sql='select DISTINCT(p.projectname),p.pid,pr.purchaseorder,pr.purchaseduedate,c."client",sr."srID"'.$join_column.' from tbl_newproject as p  inner join "clientDB" c on p.client=c."ID" left join tbl_prj_sample as sm on sm.pid =p.pid left join tbl_prj_style as style on style.pid = p.pid left join tbl_prjvendor pv on pv.pid=p.pid left join tbl_mgt_notes as notes on notes.pid = p.pid left join tbl_prjpurchase as pr on pr.pid= p.pid  where p.projectname <> \'\' '.$search_sql.' and p.created_date between '.strtotime($_REQUEST['fromDate']).' and '.$toDate.' ';
					if($search_uri) 
					{
				...
				{
					$sql=' select DISTINCT(p.projectname), p.pid,c."client",pr.purchaseorder,pr.purchaseduedate'.$join_column.' from tbl_newproject as p inner join "clientDB" c on p.client=c."ID" left join tbl_prjpurchase as pr on pr.pid= p.pid left join tbl_prj_style as style on style.pid = p.pid left join tbl_prjvendor pv on pv.pid=p.pid left join tbl_mgt_notes as notes on notes.pid = p.pid left join tbl_prj_sample as sm on sm.pid =p.pid where p.projectname <> \'\' '.$search_sql.'  ';
				}
			}
			else 
			{
                    ...
                    if(count($datalist)) 
                    {
                        for($i=0; $i < count($datalist); $i++)
                        {
                          ...
                          ?><tr>
                            ...
                            <td class="grid001"><a href="<?php echo $mydirectory;?>/admin/project_mgm/project_mgm.add.php?id=<?php echo $datalist[$i]['pid'];?>&<?php echo $paging; ?>"><?php echo $datalist[$i]['projectname'];?></a></td>
