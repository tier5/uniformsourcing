...
$sql="Select count(*) as n  from  \"tbl_sampleRequest\" where \"srID\"= '$srID' and status = 1";
if($id >0)
$sql .= " and id <> $id";
...
if($id == 0 || $id =='')
{
	$sql= "Insert into \"tbl_sampleRequest\" (";
	if($srID!="")$sql.=" \"srID\" ";
	$sql.=",status";
	$sql.=",\"createdDate\")";
	$sql.="Values(";
	...
	$sql.=",'".date('U')."')";
	if(!($result=pg_query($connection,$sql))){
		$return_arr['error'] ="Error while inserting sample request information to database!";	
		echo json_encode($return_arr);
		return;
	}
		pg_free_result($result);
	
	$sql= "Select id from  \"tbl_sampleRequest\" where \"srID\"='$srID' and status = 1";
	if(!($result=pg_query($connection,$sql))){
	...
	else 
	{
		...
		$sql = "Insert Into tbl_sample_uploads ( sampleid ";
			$sql.=", filename";		
			$sql.=",uploadtype ";
			$sql.=", status";
			$sql.=", createddate";
