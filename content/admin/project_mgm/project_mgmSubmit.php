        ...
        for (; $index < count($style); $index++)
        {
            ...
            if ($prjstyle_id[$index] != "" && $prjstyle_id[$index] > 0)
            {
                ...
                else
                {
					...
					if ($vendor_style[$index] != "")
                        $query_Name.=", vendor_style = '" . pg_escape_string($vendor_style[$index]) . "' ";
                    else
                        $query_Name.=", vendor_style = null ";
                    if ($garments[$index] != "")
    ...
    if ($proj_6 == 1)
    {
        ...
        if (count($textAreaName) > 0)
        {
            ...
            for ($i           = 0; $i < count($textAreaName); $i++)
            {
                ...
                if ($prj_note_type[$i] != "")
                    $query_Name .=" '" . pg_escape_string($prj_note_type[$i]) . "',";
                $query_Name .=" $pid";
                $query_Name .=", " . date("U");
                $query_Name .=", " . date("U");
                $query_Name .=", " . $_SESSION["employeeID"] . "";
          ...
          $query_Name = '';
           ...
           if($shipping[$i][$j]=='') $shipping[$i][$j]=0;
                    if (isset($qty_id[$i][$j]) && $qty_id[$i][$j] > 0)
                        $query_Name .= "UPDATE tbl_qty_shipped set qty_ship = '" . $shipping[$i][$j] . "',prj_style_id=".$prj_style_id[$i][$j]." where qty_id='" . $qty_id[$i][$j] . "'; ";
                    else
                        $query_Name.='insert into tbl_qty_shipped(pid,prj_style_id,shipping_id,qty_ship) values('.$pid.','.$prj_style_id[$i][$j] .',\'' .  $tracking_no  . '\',\'' . $shipping[$i][$j] . '\'); ';
            }
            //echo $query_Name."<br/>";
            if ($query_Name != "")
            {
