...
//echo $JSONLIB;
$manager_email = 'nweisman@i2net.com,nicole@i2net.com,felicita@i2net.com'; //emails send to neal,nicole,felicita
...
if (isset($_POST['pid'])) {
        ...
        for ($i = 0; $i < count($elementtype); $i++) {
            if ($element_id[$i] > 0) {
                ...
                if ($elementcost[$i] != "")
                   ...
                   if ($element_pack[$i] != "")
                    $query_Name.=", pack_id = '$element_pack[$i]' ";
                else
                    $query_Name.=", pack_id = null ";
                
                   if ($image_from[$i] != "")
                    $query_Name.=", img_type = '$image_from[$i]' ";
                else
                    $query_Name.=", img_type = null ";
                 if ($file_from[$i] != "")
                    $query_Name.=", file_type = '$file_from[$i]' ";
                else
                    $query_Name.=", img_type = null ";
                $query_Name.=", createddate = " . date('U');
                $query_Name.=", updateddate = " . date('U');
                ...
                if ($elementcost[$i] != "")
                  ...
                  if ($element_pack[$i] != 0)
                    $query_Name.=", '" . $element_pack[$i] . "' ";
                  if ($image_from[$i] != "")
                     $query_Name.=", '" . pg_escape_string($image_from[$i]) . "' ";
                  if ($file_from[$i] != "")
                      $query_Name.=", '" . pg_escape_string($file_from[$i]) . "' ";
                $query_Name.=" ,1 ";
                $query_Name.=" ,'" . date('U') . "' ";
                $query_Name.=" ,'" . date('U') . "'";
                $query_Name.=" )" . ";";
    ...
    if ($proj_8 == 1) {
        ...
        for ($i = 0; $i < count($track_shipping); $i++) {
            if ($hdn_shipping_id[$i] == 0) {
                ...
                if ($order_shipping_notes[$i] != "")
                    $query_Name.=", shipping_notes='" . $order_shipping_notes[$i] . "'";
                else
            ...
            if (($query_count * $num_of_queries <= $i) && $query_Name != "") {
                $query_count++;
                //echo $query_Name;
                if (!($result = pg_query($connection, $query_Name))) {
