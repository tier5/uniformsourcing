...
<script type="text/javascript">
    ...
    //   alert($('#upload_pack'+id).val());
        ...
        $('#element'+data.index).find('#tr_id'+data.img_count).show();
              ...
              if(data.file != '')
           {
                if($('#file_from'+(data.img_count+1)))
                  $('#file_from'+(data.img_count+1)).val("PACK");
                $('#element'+data.index).find('#file_name'+(data.img_count+1)).val(data.file);
                $('#element'+data.index).find('#tr_id'+(data.img_count+1)).html('<td ><strong>File:</strong><br/>'+
                data.file+'');
            $('#element'+data.index).find('#tr_id'+(data.img_count+1)).show();
           }
            }           
           });
...
for($i=0; $i < count($data_elements); $i++){?><option value="<?php echo $data_elements[$i]['element_id'];?>"><?php echo $data_elements[$i]['elements'];?></option><?php } ?></select></td></tr><tr><td align="right">Vendor:</td><td align="left"><select id="element_vendor" name="vendor_ID[]">	<?php for($i=0; $i < count($data_Vendr); $i++){?><option value="<?php echo $data_Vendr[$i]['vendorID']; ?>"><?php echo $data_Vendr[$i]['vendorName'];?></option><?php } ?></select></td></tr><tr><td align="right">Style:</td><td align="left"><input type="text" name="elementstyle[]" id="elementstyle" value="" /></td></tr><tr><td align="right">Color:</td><td align="left"><input type="text" name="elementcolor[]" id="elementcolor" value="" /></td></tr><tr><td align="right">Cost:</td><td align="left"><input type="text" name="elementcost[]" id="elementcost" value="" /></td></tr><tr><td align="right">Image:</td><td align="left"><input type="file" name="file'+ --count +'" id="file'+ count +'" onchange="javascript:ajaxFileUpload('+ count +', \'I\', 960,720);" /><input type="hidden" id="file_name'+ count +'" name="element_file0[]" value=""/><input type="hidden" id="upload_type'+ count +'" name="element_type0[]" value="I"/><input type="hidden" id="upload_id'+ ++count +'" name="element_id0[]" value=""/></td></tr><tr><td align="right">File:</td><td align="left"><input type="file" name="file'+ count +'" id="file'+ count +'" onchange="javascript:ajaxFileUpload('+ count +', \'F\', 960,720);" /><input type="hidden" id="file_name'+ count +'" name="element_file1[]" value=""/><input type="hidden" id="upload_type'+ --count +'" name="element_type1[]" value="F"/></td></tr></tr></table><input type="hidden" id="element_id" name="element_id[]" value="0"/><input type="hidden" id="image_from'+img_count+'" name="image_from[]" value="CURR"/><input type="hidden" id="file_from'+(img_count+1)+'" name="file_from[]" value="CURR"/></td><td valign="top" align="right"><table  border="0" cellspacing="0" cellpadding="0"><tr id="tr_id'+ count +'" style="display:none;"><td><strong>Image:</strong><br/><img id="img_file'+ count +'" width="101px" height="89px" src="" onClick="PopEx(this, null,  null, 0, 0, 50, \'PopBoxImageLarge\');" ><a style="cursor:hand;cursor:pointer;" onClick=" javascript: DeleteUploads(\'\',\''+ count +'\',\'\',\'I\',\'editTime\'); document.getElementById(\'tr_id'+ count +'\').style.display=\'none\'; document.getElementById(\'file_name'+ ++count +'\').value=\'\'; " ><img src="<?php echo $mydirectory; ?>/images/close.png" alt="delete" /></a></td>	</tr><tr id="tr_id'+ count +'" style="display:none;" ></tr></table></td></tr></table>';
//alert(in_htm);
			   ...
			   if(data.index != 'undefined' && data.index != "")
			   {
				 ...
				 else if(data.index > 100 && data.index < 200 && data.type == 'F' )
				 {
					...
					document.getElementById('tr_id'+data.index).style.display="";
				 	document.getElementById('file'+data.index).value="";
                                        if($('#file_from'+data.index))
                                         $('#file_from'+data.index).val("CURR");
				 }
