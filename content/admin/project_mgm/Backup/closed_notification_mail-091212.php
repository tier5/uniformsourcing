...
$sent_to = array();
if ($prj_data['email'] != "")
    $sent_to[0] = $prj_data['email'];

else
    $sent_to[0] = 'systems@uniforms.net';
$sent_to[count($sent_to)] = 'systems@uniforms.net';
if (isset($prj_data['email1']) && $prj_data['email1'] != '')
    $sent_to[count($sent_to)] = $prj_data['email1'];
if (isset($prj_data['email2']) && $prj_data['email22'] != '')
    $sent_to[count($sent_to)] = $prj_data['email2'];
if ($isMailServer == 'true') {
    $mail = new PHPMailer();

    $mail->AddReplyTo($prj_data['empmail'], $name = $prj_data['firstname'] . ' ' . $prj_data['lastname']);
