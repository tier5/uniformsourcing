...
$sql = "Select * from tbl_prj_elements where status =1 and pid = $pid";
   ...
   /* $sql = 'Select "elm_pack" from "tbl_newproject" where  pid ='. $pid;
	...
	$q_name='SELECT  list.pack_id,pack.pack_name from "tbl_upload_pack" as list left join tbl_element_pack_main as pack on pack.pack_id=list.pack_id  where  upload_pack_e=1 AND pid='.$pid;
...
';
$element.='<table width="80%"><tr align="left"><td align="left">Select a defult Package: '.
...
for($j = 0; $j < count($data_package); $j++)
{
 ...
 $html .'<table width="100%">';
	...
	'<td align="left">
     <input type="text" disabled="disabled" value="'.$data_package[$j]['client_name'].'"/>         
</td>'.
	'</tr>'.
	'<tr>'.
