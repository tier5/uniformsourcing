...
$q_name='SELECT  list.pack_id,pack.pack_name from "tbl_upload_pack" as list left join img_file_pack as pack on pack.pack_id=list.pack_id  where  upload_pack_u=1 AND pid='.$pid;
...
$html.='<table id="upld_pack_'.$data_package_list[$j]['pack_id'].'">';
$html.='<tr><td>Style Number: <strong>'.$data_package_list[$j]['pack_name'].
'<img  onclick="$(\'#upld_pack_'.$data_package_list[$j]['pack_id'].'\').remove()" src="../../images/delete.png" />'.
'</strong><input type="hidden" name="upload_packages[]" value="'.$data_package_list[$j]['pack_id'].'"/>'.
'</td></tr>';
if(count($imageArr_pack))
{
  $html .= "<tr><td>Images:</td></tr>";
...
if(count($imageArr))
{
  $html .= ' <tr><td> <strong>Images:</strong></td></tr>';
