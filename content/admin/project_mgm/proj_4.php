...
if(isset($_POST['pid']) && $_POST['pid']!=0){
	$isEdit=1;
	$sql = "select * from tbl_prj_style$tx  where status =1 and pid = $pid order by prj_style_id";
	...
	$sql = "Select * from tbl_prjpricing$tx where status =1 and pid = $pid";
	if(!($result=pg_query($connection,$sql))){
		print("Failed query1: " . pg_last_error($connection));
		exit;
	}
	...
	pg_free_result($result);
        ...
        $sql = "Select shiponclient from \"tbl_newproject$tx\" where pid = $pid";
if(!($result=pg_query($connection,$sql))){
		print("Failed query1: " . pg_last_error($connection));
		exit;
	}
...
<td >
<input type="text" id="vendor_style1" name="vendor_style[]"';
if($emp_type ==2){
	$html .= 'disabled="disabled"';
}
$html .= 'value="'.$data_prj_style[0]['vendor_style'].'" />
</td>
<td>
...
if($isEdit){
	for($i=1,$j=2; $i<count($data_prj_style); $i++,$j++){
		...
		<td>
		<input id="vendor_style'.$j.'" name="vendor_style[]"';
		if($emp_type ==2){
			$html .= 'disabled="disabled"';
		}
		$html .= 'type="text" value="'.htmlentities( $data_prj_style[$i]['vendor_style']).'"  />
