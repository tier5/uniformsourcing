...
if((isset($_POST['prj_name']) && $_POST['prj_name']!="") && (isset($_POST['prj_id']) && $_POST['prj_id']!=""))
{
	...
	{
		$sql = "Select * from tbl_newproject where pid = $source_pid";
		//echo $sql;
		if(!($result=pg_query($connection,$sql))){
			print("Failed sql: " . pg_last_error($connection));
			exit;
		...
		$sql = "Select * from tbl_prjpurchase where status =1 and pid = $source_pid";
		//echo $sql;
		if(!($result=pg_query($connection,$sql))){
			print("Failed sql: " . pg_last_error($connection));
			exit;
		...
		$sql = "select createddate,vid from tbl_prjvendor inner join vendor on vendor.\"vendorID\"=tbl_prjvendor.vid where status =1 and pid = $source_pid";
		//echo $sql;
		if(!($result=pg_query($connection,$sql))){
			print("Failed sql: " . pg_last_error($connection));
			exit;
		...
		$sql = "select * from tbl_prj_style where status =1 and pid = $source_pid";
		//echo $sql;
		if(!($result=pg_query($connection,$sql))){
			print("Failed sql: " . pg_last_error($connection));
			exit;
		...
		pg_free_result($result);	
		$sql = "Select * from tbl_prjsample where status =1 and pid = $source_pid";	
		//echo $sql;
		if(!($result=pg_query($connection,$sql))){
			print("Failed sql: " . pg_last_error($connection));
			exit;
		...
		$sql = "Select * from tbl_prjpricing where status =1 and pid = $source_pid";
		//echo $sql;
		if(!($result=pg_query($connection,$sql))){
			print("Failed sql: " . pg_last_error($connection));
			exit;
	...
	//	echo $sql;	
		...
		$sql = "Select * from tbl_prmilestone where pid = $source_pid and status = 1";	
		//echo $sql;
		if(!($result=pg_query($connection,$sql))){
			print("Failed sql: " . pg_last_error($connection));
			exit;
		...
		$sql = "Select * from tbl_prj_elements where status =1 and pid = $source_pid";
		//echo $sql;
		if(!($result=pg_query($connection,$sql))){
			print("Failed sql: " . pg_last_error($connection));
			exit;
		...
		$sql = "Select tbl_mgt_notes.*,e.firstname as \"firstName\", e.lastname as \"lastName\" from tbl_mgt_notes inner join \"employeeDB\" as e on e.\"employeeID\" =tbl_mgt_notes.\"createdBy\"  where \"isActive\" =1 and pid = $source_pid";
		//echo $sql;
		if(!($result=pg_query($connection,$sql))){
			print("Failed sql: " . pg_last_error($connection));
			exit;
		...
		$sql = "Select * from tbl_prjimage_file where status =1 and pid = $source_pid";
		//echo $sql;
		if(!($result=pg_query($connection,$sql))){
			print("Failed sql: " . pg_last_error($connection));
			exit;
		...
		$sql = "select tbl_prjorder_shipping.*,tbl_carriers.weblink from  tbl_prjorder_shipping left join tbl_carriers on tbl_carriers.carrier_id = tbl_prjorder_shipping.carrier_id where tbl_prjorder_shipping.status=1 and pid = $source_pid";
		//echo $sql;
		if(!($result=pg_query($connection,$sql))){
			print("Failed sql: " . pg_last_error($connection));
			exit;
