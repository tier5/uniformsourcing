...
<img id="bar_img" width="50" height="50" src="../uploadFiles/inventory/images/<?php echo $data_style['barcode'];?>" onclick="PopEx(this, null,  null, 0, 0, 50,'PopBoxImageLarge');">
...
$.ajaxFileUpload({url:'doajaxfileupload.php?field='+filefield+'&fileType='+filetype+'&projectId='+id<?php if($isEdit) {?>+'&colorId='+clrId<?php }?>,secureuri:false,fileElementId:filefield,dataType: 'json',
    ...
    $("#barcode_name").val($("#barcode_image").val());
    $("#bar_img").attr("src","../uploadFiles/inventory/images/"+$("#barcode_image").val());
...
if(clrId > 0)
{
	cell6.innerHTML = "<img id=\"image_"+clrId+"\" src=\"<?php echo $upload_dir_image;?>"+imgName+"\" alt=\"thumbnail\" width=\"64\" height=\"48\" border=\"0\" />";
}
else
{
	cell6.innerHTML = "<img src=\"<?php echo $upload_dir_image;?>"+imgName+"\" alt=\"thumbnail\" width=\"64\" height=\"48\" border=\"0\" />";
}
...
if($isEdit){
	for($i=0;$i < count($data_color);$i++)
		echo "AddRow('imageView','".$data_color[$i]['name']."','".$data_color[$i]['image']."',".$data_color[$i]['colorId']." );";
}
?>
...
$(function(){$("#validationForm").submit(function(){dataString = $("#validationForm").serialize();if(document.getElementById('isEdit').value==1)dataString += "&type=e";else dataString += "&type=a";$.ajax({type: "POST",url: "styleSubmit.php",data: dataString,dataType: "json",success:
