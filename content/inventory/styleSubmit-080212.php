			...
			case "a":
			{
				...
				if($row[0] != 0)
				{
					...
					return;
                                ...
                                $path_parts = pathinfo($barcode_name);
				...
				if($return_arr['id'] != "")
				{
					for($i=0; $i < count($clolorName); $i++)
					{
						...
						if($clolorName[$i] != "")$query1.=", '$clolorName[$i]' ";
						$query1.=", '$imageName[$i]')";						
						if(!($result=pg_query($connection,$query1))){
							$return_arr['error'] ="Error while storing style information to database!";	
							echo json_encode($return_arr);
				...
				if($styleId > 0)
				{
					...
					for(; $index < count($clolorName); $index++)
					{
						...
						if($index < count($data_color))
						{
							...
							$query .= ", \"name\" = '$clolorName[$index]' ";
							$query .= ", \"image\" = '$imageName[$index]' ";
							$query .= "where \"colorId\"='".$data_color[$index]['colorId']."' ";							
						}
						else
						{
							...
							$query.=", \"image\" ) VALUES(";
							$query.=" '$styleId' ";
							if($clolorName[$index] != "")$query.=", '$clolorName[$index]' ";
							$query.=", '$imageName[$index]')";
						}						
						if(!($result=pg_query($connection,$query))){
							$return_arr['error'] = pg_last_error($connection);
							echo json_encode($return_arr);
