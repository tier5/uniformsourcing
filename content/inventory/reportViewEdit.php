...
<table width="90%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td align="left"><input type="button" value="Back" onclick="location.href='reports.php'" />
   <?php echo $_SESSION['perm_admin']; ?>
    </td>
    <td>&nbsp;</td>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	  <td width="10"></td>
          ...
          <tr>
            <td><img id="imgView" src="<?php echo $upload_dir_image.trim($imageName);?>" alt="thumbnail" width="150" height="230" border="1" class="mouseover_left" /></td>
            </tr>
          <tr>
            <td height="100">&nbsp;</td>
            </tr>
...
$("#inventoryForm").submit(function(){
 ...
 var dataString = $("#inventoryForm").serialize();
 dataString += "&type=e";
 $.ajax({type: "POST",url: "invReportSubmit.php",
     data: dataString,dataType: "json",success:function(data){
         if(data!=null){
      if(data[0].name || data[0].error){
      $("#message").html("<div class='errorMessage'><strong>Sorry, " + data[0].name + data[0].error +"</strong></div>"); if(data[0].flag){$(location).attr('href',"storage.php?type=a&styleId="+document.getElementById('styleId').value+"&colorId="+document.getElementById('colorId').value);} } else {if(data[0].flag){$("#message").html("<div class='successMessage'><strong>Inventory Quantity Updated. Thank you.</strong></div>");
      
          $(location).attr('href',"storage.php?type=a&styleId="+document.getElementById('styleId').value+"&colorId="+document.getElementById('colorId').value);}else{$("#message").html("<div class='successMessage'><strong> All inventorys are uptodate...</strong></div>");
      location.reload();    
  }}}else{$("#message").html("<div class='errorMessage'><strong>Sorry, Unable to process.Please try again later.</strong></div>");}}
	  });
	//  return false; 
      }
	  else{
